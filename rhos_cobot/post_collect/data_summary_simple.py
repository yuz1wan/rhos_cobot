...
def process_folder(folder_path, task_id, user_id, scene_id):
    """
    处理指定文件夹，重命名文件夹并压缩成tar.gz
    ...
    # 获取所有hdf5文件
    hdf5_files = [f for f in os.listdir(folder_path) if f.startswith('episode_') and f.endswith('.hdf5')]
...
def main():
    ...
    try:
        process_folder(folder_path, task_id, user_id, scene_id)
    except Exception as e:
        print(f"错误: {str(e)}")
