...
def save_data(args, timesteps, actions, actions_eef, dataset_path):
    ...
    with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024**2*2) as root:
        ...
        for cam_name in args.camera_names:
            # import pdb
            # pdb.set_trace()
            enc_imgs, max_len = images_encoding(data_dict[f'/observations/images/{cam_name}'], mode='color')
        ...
        if args.use_depth_image:
            ...
            for cam_name in args.camera_names:
                enc_depths, max_len_depth = images_encoding(data_dict[f'/observations/images_depth/{cam_name}'], mode='depth')
                data_dict[f'/observations/images_depth/{cam_name}'] = enc_depths
                dt_depth = f'S{max_len_depth}'
                image_depth.create_dataset(cam_name, (data_size,), dtype=dt_depth)
...
class RosOperator:
    ...
    def get_frame(self):
        if len(self.img_left_deque) == 0 or len(self.img_right_deque) == 0 or len(self.img_front_deque) == 0 or \
            ...
            # print("len(self.img_left_deque):", len(self.img_left_deque))
            # print("len(self.img_right_deque):", len(self.img_right_deque))
            # print("len(self.img_front_deque):", len(self.img_front_deque))
            # if self.args.use_depth_image:
            #     print("len(self.img_left_depth_deque):", len(self.img_left_depth_deque))
            #     print("len(self.img_right_depth_deque):", len(self.img_right_depth_deque))
            #     print("len(self.img_front_depth_deque):", len(self.img_front_depth_deque))
            return False
        if self.args.use_depth_image:
    ...
    def init_ros(self):
        ...
        rospy.Subscriber(self.args.puppet_eef_left_topic, PoseStamped, self.puppet_eef_left_callback, queue_size=1000, tcp_nodelay=True)
        rospy.Subscriber(self.args.puppet_eef_right_topic, PoseStamped, self.puppet_eef_right_callback, queue_size=1000, tcp_nodelay=True)
    ...
    def process(self):
        ...
        while (count < self.args.max_timesteps + 1) and not rospy.is_shutdown():
            ...
            # 2.2 可视化三个视角的图像
            combined_image = np.concatenate((cv2.cvtColor(img_left, cv2.COLOR_BGR2RGB), cv2.cvtColor(img_front, cv2.COLOR_BGR2RGB), cv2.cvtColor(img_right, cv2.COLOR_BGR2RGB)), axis=1)
...
#图像读取方式
# img_rgb = cv2.imdecode(np.frombuffer(jpeg_bytes, np.uint8), cv2.IMREAD_COLOR)
# img_depth = cv2.imdecode(np.frombuffer(png_bytes, np.uint8), cv2.IMREAD_UNCHANGED)  # 保留16位精度

# python collect_data_eef.py --dataset_dir ./data --max_timesteps 500 --episode_idx 0
